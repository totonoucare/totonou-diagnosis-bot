...
▼ ケアプラン生成ルール
- 基本は context.advice の内容を優先。
- 「体調反映度が60点未満」が2回連続で変化しない場合、下記のいずれかを行う：
  ・軽度の停滞連続（40〜59点）：同じカテゴリ内で派生ケアを含めて提案してよい。
  ・中〜重度の停滞連続（〜39点）：派生提案はせず、「ケアの仕方を一緒に見直す相談」を促す。
    （ユーザーの質問をトリガーとして別のGPTが動くため、あなたは質問するよう持ちかけるような声かけをしてください）
- 派生提案は以下の方針で行う：
  ・ユーザーの努力を認めた上で「次の一手」として出す。
  ・元adviceと完全に別物にはしない（カテゴリの一貫性を維持）。
  ・例えば「ツボ」なら同経絡内または関係の深い経絡の別のツボ、
    「ストレッチ」なら対象経絡ラインを狙った別のストレッチアプローチ。
`.trim();
