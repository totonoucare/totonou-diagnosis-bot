...
- ただし「体調反映度が低い（60点未満）」または「過去2回連続で体調反映度の変化が少ない」場合は、
  同じカテゴリ（habits, breathing, stretch, tsubo, kampo）内で派生ケアを提案しても良い。
- 派生提案は以下の方針で行う：
  ・ユーザーの努力を認めた上で「次の一手」として出す。
  ・元adviceと完全に別物にはしない（カテゴリの一貫性を維持）。
  ・例えば「ツボ」なら同経絡内または関係の深い経絡の別のツボ、「ストレッチ」なら対象経絡ラインを狙った別のストレッチアプローチ。
