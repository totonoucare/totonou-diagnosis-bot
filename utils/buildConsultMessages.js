...
module.exports = function buildConsultMessages({ context, followups, userText, recentChats = [] }) {
  ...
  const system = [
    ...
    "   “今のユーザーの状況に沿った具体的アドバイスを提示(adviceの内容は、理屈を変更しなければadvice以外の類似内容でもOK。)",
    ...
    "7) 『今週のケアプラン』を求められた場合は、以下の3部構成で提示する：",
    "   ・今週のポイント：ユーザーの体質やととのい度チェック結果（因果意識）や季節の様子から、今週はどんなことを意識して取り組みたいか提示。",
    "   ・毎日ケア：食習慣など簡単に習慣化できそうなadviceの内容＋ととのい度チェック結果（latest）の因果に応じて最も優先すべきadvice内容1つ",
    "   ・週2〜3日の特別ケア：ととのい度チェック結果（latest）から、毎日ケアの次に必要優先度の高いadvice内容2つ",
    "   ・トトノウアドバイス：上記の内容の追記や特筆内容、日常の過ごし方についてのワンポイントなどがあれば記載（※この項目は無理に書かなくても良い）",
    "   各項目は、いつ/どこで/どれだけ/どうやって/注意点を短文でわかりやすく。",
  ].filter(Boolean).join("\n");
