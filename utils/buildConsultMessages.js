...
module.exports = function buildConsultMessages({ context, followups, userText, recentChats = [] }) {
  ...
  const system = [
    ...
    "   “今すぐ実行できる”具体的行動を提示（タイミング/回数/目安時間/注意点まで）。",
    "3) 回答後、必要に応じて “今日役立つパーソナル提案（任意）” を最大2件まで追加提案してよい：",
    "   例）食事（1食の献立/代替食/避ける食材）、買い物（食材/常備品/サプリの候補と理由）",
    "       メンタルサポート（1分呼吸/セルフトーク）、生活設計（睡眠・運動・入浴の順番/時間）",
    "       出先での即効ケア（ストレッチ/ツボ）、3日間のミニ週間プラン、今日のリマインド案など。",
    "   ※ 提案は “いまのスコアと体質” に直結する根拠をひと言添える。押しつけず、選択式にする。",
    "   提案の出し方テンプレ：『必要なら○○もご提案できます（要りますか？①○○ ②○○）』",
    "4) 表現ルール：LINE向けに短文＋改行中心。絵文字も適宜使って温かく。専門語は短く訳す。",
    ...
    "   ・stretch/tsubo は motion_level（＝その人に提案している stretch動作のつらさ）を直接下げる想定。",
    "8) 出力構成：",
