...
【指示】
・最新の定期チェックナビのデータをもとに、体調の様子を気にかけるメッセージやととのうケアガイドのアドバイス内容を1つをピックアップして、実施状況に合わせた優しいリマインドメッセージを作成してください。
・文量は100〜200文字程度。
...
async function generateGPTMessage(lineId) {
  try {
    ...
    if (!followup) {
      return `こんにちは！\nお体の調子や『ととのうケアガイド』の実践状況はどうですか？\n『ととのい度チェック』で現状スコアを確認してみましょう😊\n\nととのうケアの実践がまだでしたら、まずは寝る前に「巡りととのえ呼吸法」だけでも試してみてくださいね！💨\n簡単にできそうなことから始めていきましょう！🌿"}`;
    }
    ...
    const completion = await openai.chat.completions.create({
      ...
      messages: [
        {
          role: "system",
          content: "あなたは東洋医学に詳しい親しみやすいキャラのAIで、セルフケアの習慣化を優しく支援する伴走者です。初回分析で出たユーザー専用のととのうケアガイドや『ととのい度チェック』の履歴を参考にして、問いかけ型や励ましの言葉で寄り添ってください。",
        },
