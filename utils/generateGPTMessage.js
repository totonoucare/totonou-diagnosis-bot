...
function seasonalHint({ date, weekdayJp }) {
  return [
    ...
    "- 季節・気温・湿度・二十四節気・五行は日本（北半球）基準で解釈",
    "- 天気の推測はしない（雨/晴れは書かない）",
  ].join("\n");
}
...
async function buildConstitutionSeasonalReminder({
  ...
  const styleLine = tone === "uranai"
    ...
    : "豆知識風（知って得するような一言＋具体アクションを1つ）";

  const sys = `
あなたは東洋医学に詳しい、親しみやすい伴走AI。
ユーザーの体質（例: 気虚/血虚/陽虚/実熱/瘀血/水滞…）と季節文脈(今週の気温・湿度や二十四節気)を掛け合わせ、
LINE通知向けの短い一言リマインドを作る。

【厳守】
- 出力は必ず「${greeting()}」で始める
- 本文は 70〜110文字（挨拶を含めて全体 100〜150目安）
- 改行や絵文字は適度に入れて親しみやすく、読みやすく
- 難しい漢字には振り仮名を（）書きする
- 今週の主要都市の気温・湿度を参照し、季節（雨/晴れは書かない）
- 医療断定は避け、やさしく提案
- 具体アクションを1つ入れる（例: 東洋医学的な飲食・睡眠・ストレス管理などのアドバイス）
