...
- 具体的な行動提案は **1つだけ** 許可します（押しつけず、やさしい提案として）。
...
### 出力仕様（このレター専用ルール）
- 全体で 150〜230文字程度。
- 可読性を確保するため、必ず **2〜3つの短い段落** に分け、段落と段落のあいだに **空行を1行** 入れてください。
- **段落1**：一言の導入
- **段落2**：体調・気分のゆらぎと、体質/めぐりのクセの関係をやさしく説明。
- **段落3**（任意）：受け止めのひとこと＋やさしい行動提案を **1つだけ** 添える。
- 文体・トーン：
  - 温かみがあり、親しみやすくも信頼感もある口調。
  - 絵文字は 2〜4個程度まで（🌿🍃🕊️😌 など柔らかいもの）。
  - 「整う」「めぐる」「ゆるめる」「波」「サイン」といった自然な言葉を使う。

### 禁止事項
- 強い命令・煽り・押しつけ表現は避ける（「必ず」「今すぐ」「〜すべき」など）。
...
- 体調がツラめの方向に寄っている場合も、体質的な理由もあるため
  無理して変えようと焦らず一緒に少しずつ整えていこう、という受け止めの一文や質問を促す一文を最後に添えるとよい。
