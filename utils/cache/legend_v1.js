...
module.exports = `
▼ ととのい度チェックとは？
・週に一回ペース程度で体の状態を数値で自己評価するチェックです(ボタンでいつでも開始できます)。symptom_level（主訴の強さ）と、生活リズム指標（sleep / meal / stress）および動作時のつらさ（motion_level）を1〜5で入力します。
...
・ユーザーには実施総数が表示されますが、内部ロジックでは同一日に同じケアを複数回行っても1回として丸めて扱われます。(日数頻度を見るため)
